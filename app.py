...
# Try loading .env (optional — not needed on Streamlit Cloud)
try:
...
<style>
    ...
    .api-status-ok { color: #6ee7b7; font-weight: 600; }
    .api-status-missing { color: #fca5a5; font-weight: 600; }
</style>
...
with st.sidebar:
    ...
    # Set the key globally when provided
    if api_key_input:
        os.environ["GROQ_API_KEY"] = api_key_input
        st.markdown('<span class="api-status-ok">✅ API Key configured</span>', unsafe_allow_html=True)
    else:
        st.markdown('<span class="api-status-missing">⚠️ Enter your API key to chat</span>', unsafe_allow_html=True)
 ...
 └───────────┘
    ...
    st.divider()
    st.caption("Built with LangGraph + Groq • [GitHub](https://github.com/ayushchhipa1509/xplora-ai)")
...
if not api_key_input and not os.getenv("GROQ_API_KEY"):
    st.info("👈 **Enter your Groq API key in the sidebar to start chatting.** Get a free key at [console.groq.com](https://console.groq.com)")
